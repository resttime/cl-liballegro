...
3. Enums/constants are shortened, check [[./src/constants][constants]] if you need help finding them
4. Type names have changed too, check [[./src/types][types]] if you need help finding them
5. To access slots from a C struct, you can use CFFI:MEM-REF create a plist
 ...
 #+END_SRC
6. I've got a neat *OPTIONAL* [[./src/interface/interface.lisp][lispy interface]] which provides an entire fixed timestep game loop
...
9. [[./examples][Examples]] exist if you get lost
...
[[./examples/001-simple-window.lisp][example]]
...
multiple namespaces to handle naming clashes.  There are a few edge
cases where naming clashes were unavoidable, check the source code to
be sure.
...
and not optimized.  Multiple dispatch is slower than static dispatch.
...
[[./src/interface]]/: Common Lisp interface definition, optional

[[./src/package.lisp]]: Common Lisp package defintion, exports usable symbols
